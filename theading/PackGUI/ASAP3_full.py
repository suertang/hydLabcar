    ...
    def feed_response(self, resp):
        if resp["cmd"] != self.cmd:
            raise NotImplementedError(
                "get_response {0} but found {1}".format(self.cmd, resp["cmd"]))
...
class asap3select_desc_and_bin(asap3service):
    def __init__(self, desc_file, bin_file, dest):
        ...
        super(asap3select_desc_and_bin, self).__init__(
            cmd="select_description_file_and_binary_file", data=data)
...
class asap3define_desc_and_bin(asap3service):
    def __init__(self, desc_file, prog_file, cal_file, dest, mode):
        ...
        super(asap3define_desc_and_bin, self).__init__(
            cmd="define_description_and_binary_file", data=data)
...
class asap3change_bin_name(asap3service):
    def __init__(self, new_name, Lun):
        ...
        super(asap3change_bin_name, self).__init__(
            cmd="change_binary_filename", data=data)
...
class asap3select_lookup_table(asap3service):
    def __init__(self, Lun, map_name):
        ...
        super(asap3select_lookup_table, self).__init__(
            cmd="select_look-up_table", data=data)
...
class asap3extended_select_lookup_table(asap3service):
    def __init__(self, Lun, map_name):
        ...
        data.extend(struct.pack(">H", Lun))
        data.extend(create_asap3_string(s=map_name))
        super(asap3extended_select_lookup_table, self).__init__(
            cmd="extended_select_look-up_table", data=data)

    def feed_specific(self, resp):
        data = resp.pop("data")
        map_num, ny, nx, y_phys_data_type, x_phys_data_type, z_phys_data_type, y_ctl_data_type, x_ctl_data_type, z_ctl_data_type, addr = struct.unpack(
            ">H"*9+"i", data)
        resp.update({"map_number": map_num,
                     "y_number": ny,
                     "x_number": nx,
...
class asap3get_lookup_table(asap3service):
    def __init__(self, map_number):
        ...
        super(asap3get_lookup_table, self).__init__(
            cmd="get_look-up_table", data=data)
...
class asap3put_lookup_table(asap3service):
    def __init__(self, map_number, vals):
        ...
        super(asap3put_lookup_table, self).__init__(
            cmd="put_look-up_table", data=data)
...
class asap3get_lookup_table_value(asap3service):
    def __init__(self, map_number, y_idx, x_idx):
        ...
        super(asap3get_lookup_table_value, self).__init__(
            cmd="get_look-up_table_value", data=data)
...
class asap3increase_lookup_table(asap3service):
    def __init__(self, map_number, y_idx, x_idx, y_delta, x_delta, offset):
        data = bytearray()
        data.extend(struct.pack(">HHHHHf", map_number,
                                y_idx, x_idx, y_delta, x_delta, offset))
        super(asap3increase_lookup_table, self).__init__(
            cmd="increase_look-up_table", data=data)
...
class asap3set_lookup_table(asap3service):
    def __init__(self, map_number, y_idx, x_idx, y_delta, x_delta, val):
        data = bytearray()
        data.extend(struct.pack(">HHHHHf", map_number,
                                y_idx, x_idx, y_delta, x_delta, val))
        super(asap3set_lookup_table, self).__init__(
            cmd="set_look-up_table", data=data)
...
class asap3parameter_for_value_aquisition(asap3service):
    def __init__(self, Lun, sample_rate, val_names):
        ...
        super(asap3parameter_for_value_aquisition, self).__init__(
            cmd="parameter_for_value_acquisition", data=data)
...
class asap3switching_online_offline(asap3service):
    def __init__(self, mode):
        data = bytearray()
        data.extend(struct.pack(">H", mode))
        super(asap3switching_online_offline, self).__init__(
            cmd="switching_online_offline", data=data)
...
class asap3get_parameter(asap3service):
    def __init__(self, Lun, para_name):
        ...
        super(asap3get_parameter, self).__init__(
            cmd="get_parameter", data=data)
...
class asap3set_parameter(asap3service):
    def __init__(self, Lun, para_name, val):
        ...
        super(asap3set_parameter, self).__init__(
            cmd="set_parameter", data=data)
...
class asap3extended_get_parameter(asap3service):
    def __init__(self, Lun, para_name):
        ...
        data.extend(struct.pack(">H", Lun))
        data.extend(create_asap3_string(s=para_name))
        super(asap3extended_get_parameter, self).__init__(
            cmd="extended_get_parameter", data=data)
...
class asap3extended_set_parameter(asap3service):
    def __init__(self, Lun, para_name, val, data_type=None):
        ...
        if data_type == 0:
            ...
            data.extend(create_asap3_string(s=val))
        super(asap3extended_set_parameter, self).__init__(
            cmd="extended_set_parameter", data=data)
...
class asap3set_graphic_mode(asap3service):
    def __init__(self, mode):
        data = bytearray()
        data.extend(struct.pack(">H", mode))
        super(asap3set_graphic_mode, self).__init__(
            cmd="set_graphic_mode", data=data)
...
class asap3set_case_sensitive_labels(asap3service):
    def __init__(self):
        super(asap3set_case_sensitive_labels, self).__init__(
            cmd="set_case_sensitive_labels")


class asap3define_recorder_parameters(asap3service):
    def __init__(self, Lun, rec_type, sample_rate, divider, val_names):
        data = bytearray()
        data.extend(struct.pack(">HHfHH", Lun, rec_type,
                                sample_rate, divider, len(val_names)))
        for val in val_names:
            data.extend(create_asap3_string(s=val))
        super(asap3define_recorder_parameters, self).__init__(
            cmd="define_recorder_parameters", data=data)
...
class asap3define_trigger_condition(asap3service):
    def __init__(self, start_trigger, stop_trigger, sample_count, start_delay, stop_delay):
        ...
        super(asap3define_trigger_condition, self).__init__(
            cmd="define_trigger_condition", data=data)
...
class asap3activate_recorder(asap3service):
    def __init__(self, mode):
        data = bytearray()
        data.extend(struct.pack(">H", mode))
        super(asap3activate_recorder, self).__init__(
            cmd="activate_recorder", data=data)


class asap3get_recorder_status(asap3service):
    def __init__(self):
        super(asap3get_recorder_status, self).__init__(
            cmd="get_recorder_status")
...
class asap3get_recorder_result_header(asap3service):
    def __init__(self):
        super(asap3get_recorder_result_header, self).__init__(
            cmd="get_recorder_result_header")

    def feed_specific(self, resp):
        data = resp.pop("data")

        starttime, data = pop_asap3_string(data)
        rec_type, sample_rate, divider, sample_cnt, start_delay, stop_delay, lost_samples_cnt, max_phase_error = struct.unpack(
            ">HfHIIIII", data)
...
class asap3get_recorder_results(asap3service):
    def __init__(self, sample_number):
        ...
        super(asap3get_recorder_results, self).__init__(
            cmd="get_recorder_results", data=data)
...
class asap3save_recorder_file(asap3service):
    def __init__(self, fname):
        ...
        super(asap3save_recorder_file, self).__init__(
            cmd="save_recorder_file", data=data)
...
class asap3load_recorder_file(asap3service):
    def __init__(self, fname):
        data = bytearray()
        data.extend(create_asap3_string(s=fname))
        super(asap3load_recorder_file, self).__init__(
            cmd="load_recorder_file", data=data)
...
class asap3extended_query_available_services(asap3service):
    def __init__(self):
        super(asap3extended_query_available_services, self).__init__(
            cmd="extended_query_available_services")
...
class asap3extended_get_service_information(asap3service):
    def __init__(self, service):
        ...
        super(asap3extended_get_service_information, self).__init__(
            cmd="extended_get_service_information", data=data)
...
class asap3extended_execute_service(asap3service):
    def __init__(self, service, service_input_parameter):
        ...
        super(asap3extended_execute_service, self).__init__(
            cmd="extended_execute_service", data=data)
...
class asap3client:
    def __init__(self, host=None, port=None, timeout=30):
        ...
        self.description = "python3 client for ASAP3 V{0}".format(
            ASAP3CLIENTVERSION)
    ...
    def transmit(self, msg):
        self._log_debug("SND: {0}".format(
            " ".join(["{0:02X}".format(x) for x in msg])))
    ...
    def connect_to_host(self, host, port):
        ...
        if resp["status"] == 0:
            ...
            if resp["status"] == 0:
                self._log_info(
                    "Server:{description}, Version:{version}".format_map(resp))
                self.remote_server_description = str(resp["description"])
                self.remote_server_asap_version = float(resp["version"])
                assert (self.remote_server_asap_version ==
                        self.implemented_asap_version)
    ...
    def handlerequests(self):
        ...
        while (self.con != None):
            ...
            while not self.currentrequest.is_complete():
                self.currentrequest.feed_response(
                    self.rx_queue.get(timeout=self.timeout))
        return None
    ...
    def handlerx(self):
        ...
        while (self.con != None):
            ...
            if len(data) == 2:
                ...
                self.rxbuffer.extend(data)
                self._log_debug("RCV: {0}".format(
                    " ".join(["{0:02X}".format(x) for x in data])))
    ...
    def a3select_desc_and_bin(self, desc_file, bin_file, dest):
        self._log_debug("a3select_desc_and_bin w desc_file {0} bin_file {1} dest {2}".format(
            desc_file, bin_file, dest))
    ...
    def a3define_desc_and_bin(self, desc_file, prog_file="", cal_file="", dest=0, mode=0):
        self._log_debug("a3define_desc_and_bin w desc_file {0} prog_file {1} cal_file {2} dest {3} mode {4}".format(
            desc_file, prog_file, cal_file, dest, mode))
    ...
    def a3copy_bin(self, tgt=2, src=4, Lun=0):
        self._log_debug(
            "a3copy_bin w tgt {0} src {1} Lun {2}".format(tgt, src, Lun))
    ...
    def a3get_lookup_table_value(self, map_number, y_idx, x_idx):
        self._log_debug("a3get_lookup_table_value w map_number {0} y_idx {1} x_idx {2}".format(
            map_number, y_idx, x_idx))
    ...
    def a3increase_lookup_table_value(self, map_number, y_idx, x_idx, y_delta, x_delta, offset):
        self._log_debug("a3increase_lookup_table_value w map_number {0} y_idx {1} x_idx {2} y_delta {3} x_delta {4} offset {5}".format(
            map_number, y_idx, x_idx, y_delta, x_delta, offset))
    ...
    def a3set_lookup_table_value(self, map_number, y_idx, x_idx, y_delta, x_delta, val):
        self._log_debug("a3set_lookup_table_value w map_number {0} y_idx {1} x_idx {2} y_delta {3} x_delta {4} offset {5}".format(
            map_number, y_idx, x_idx, y_delta, x_delta, val))
    ...
    def a3parameter_for_value_aquisition(self, Lun, sample_rate, val_names):
        self._log_debug("a3parameter_for_value_aquisition w Lun {0} sample_rate {1} vals {2}".format(
            Lun, sample_rate, val_names))
    ...
    def a3get_parameter(self, Lun, para_name):
        self._log_debug(
            "a3get_parameter w Lun {0} para_name {1}".format(Lun, para_name))
        return self.request_service(s=asap3get_parameter(Lun=Lun, para_name=para_name))

    def a3set_parameter(self, Lun, para_name, val):
        self._log_debug(
            "a3set_parameter w Lun {0} para_name {1} val {2}".format(Lun, para_name, val))
    ...
    def a3extended_select_lookup_table(self, Lun, map_name):
        self._log_debug(
            "a3a3extended_select_lookup_table w Lun {0} map_name {1}".format(Lun, map_name))
    ...
    def a3extended_get_parameter(self, Lun, para_name):
        self._log_debug(
            "a3extended_get_parameter w Lun {0} para_name {1}".format(Lun, para_name))
    ...
    def a3extended_set_parameter(self, Lun, para_name, val, data_type=None):
        self._log_debug("a3extended_set_parameter w Lun {0} para_name {1} val {2}".format(
            Lun, para_name, val))
    ...
    def a3extended_get_service_information(self, service):
        self._log_debug(
            "a3extended_get_service_information w service {0}".format(service))
    ...
    def a3extended_execute_service(self, service, service_input_parameter):
        self._log_debug("a3extended_execute_service w service {0} service_input_parameter {1}".format(
            service, service_input_parameter))
    ...
    def get_map_vals_by_name(self, map_name, Lun=0):
        self._log_debug(
            "get_map_vals_by_name w map_name {0} Lun {1}".format(map_name, Lun))
        ...
        if zsl != ezsl:
            raise ValueError(
                "Incorrect Length of Z Values {0} but should be {1}".format(zsl, ezsl))
        return {"x_vals": xs,
                "y_vals": ys,
                "z_vals": zs,
                "z_max": z_max,
    ...
    def set_map_vals_by_name(self, map_name, x_vals, z_vals, y_vals=None, Lun=0):
        self._log_debug("get_map_vals_by_name w map_name {0} x_vals {1} y_vals {2} z_vals {2} Lun {3}".format(
            map_name, x_vals, z_vals, y_vals, Lun))
    ...
    def set_whole_map_vals(self, map_info, val_z, Lun=0):
        ...
        nx = map_info["x_number"]
        ny = map_info["y_number"]
        addr = map_info["address"]
        z_max = map_info["z_max"]
        z_min = map_info["z_min"]
        min_inc = map_info["min_inc"]
        xs = map_info["x_vals"]
        ys = map_info["y_vals"]
        zs = [val_z]*nx*ny
        # print(resp["z_vals"])
    ...
    def record_online_values(self, Lun, val_names, sample_rate, duration):
        try:
            resp = self.a3parameter_for_value_aquisition(
                Lun=Lun, sample_rate=sample_rate, val_names=val_names)
        except asap3error:
            self._log_error(
                "read_online_values failed at parameter_for_value_aquisition {status},{err_code},{err_txt}".format_map(resp))

        try:
            self.switch_online()
        except asap3error:
            self._log_error(
                "read_online_values failed at switch_online {status},{err_code},{err_txt}".format_map(resp))
        else:
            ...
            while time.time() < endtime:
                try:
                    ...
                    self._log_error(
                        "read_online_values failed at get_online_values {status},{err_code},{err_txt}".format_map(resp))
                    break
                else:
                    v = resp["vals"]
            ...
            try:
                ...
                self._log_error(
                    "read_online_values failed at switch_offline {status},{err_code},{err_txt}".format_map(resp))

        val_dict = {}
        for idx, val_name in enumerate(val_names):
            val_dict.update({val_name: [online_vals[i][idx]
    ...
    def remote_record_values(self, Lun, val_names, rec_type=0, sample_rate=100, divider=0, trigger_condition_start="", start_trigger="", stop_trigger="", sample_count=0, start_delay=0, stop_delay=0, fname="", duration=None):
        ...
        resp = self.a3define_recorder_parameters(
            Lun=Lun, rec_type=rec_type, sample_rate=sample_rate, divider=divider, val_names=val_names)

        resp = self.a3define_trigger_condition(start_trigger=start_trigger, stop_trigger=stop_trigger,
                                               sample_count=sample_count, start_delay=start_delay, stop_delay=stop_delay)
        ...
        for idx, val in enumerate(val_names):
            val_dict.update({val: [record_vals[i][idx]
...
def selftest(testmode="object"):
    ...
    if testmode == "object":
        ...
        map_info = mya3client.get_map_vals_by_name("DEMO_CURVE")
        ...
        mya3client.a3activate_recorder(2)
        time.sleep(10)
