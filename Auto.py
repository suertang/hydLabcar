...
import time
import datetime
import queue # for python2.x this should be Queue
import threading
import logging
...
import serial
#import time
import _thread # for python2.x this should be thread
############
A3_INIT = 0x1000000
A3_REQ = 0x1000001
...
def pop_asap3_string(data):
    ...
    if L % 2:
        L += 1 # if length(byte) is odd, "0" also need to be counted
...
def create_asap3_message(cmd,status=None,data=None):
    ...
    if data:
        if isinstance(data,str):
            ...
            d.extend(data)
    ret = bytearray()
    ret.extend(struct.pack(">H",len(d)+4)) # add the lenth of checksum 4 bytes
    ret.extend(d)
    ...
    def read_data(self):
        while True:
            ...
            if(data_str!=""):                
                ...
                elif(len(data_str)>0 and data_str[0]=='\n'):
...
def selftest1(testmode="object"):
    HOST = "10.8.187.241"
    PORT = 22222
    
    ####
    #cmd = raw_input("Please input 'start' and hit enter")
    if testmode == "object":    
        #init asap3
        ...
        #init DZE
        mSerial=MSerialPort('COM1',9600)
        time.sleep(0.1)
        _thread.start_new_thread(mSerial.read_data,())
        time.sleep(0.1)
        _thread.start_new_thread(mSerial.send_heartbeat,())
        time.sleep(0.1)
        mSerial.send_data("\nRs\r")
        time.sleep(0.1)
        mSerial.send_data("\nRn\r")
        time.sleep(0.1)
        print("Speed Ready!")
        for line in open("Speedinput_Python.csv"):
            [sp,q]=line.strip().split(';')
            if(sp!="Speed"):
                resp=mya3client.a3set_parameter(0,"InjCrv_qMI1Des_C",float(q))
                mSerial.send_data("\nG" + tohaus(float(sp)) +"\r")
                time.sleep(0.1)
                #print(resp)
        mSerial.send_data("\nG0\r")
...
def selftest(testmode="object"):
    ...
    if testmode == "object":
        ...
        map_info=mya3client.a3select_lookup_table(0,"DEMO_MAP_1")
        try:
            mya3client.a3set_lookup_table_value(map_info["map_number"],1,1,map_info["y_number"],map_info["x_number"],40)
        #remote_server_desc = mya3client.get_remote_server_data()[0].upper()
